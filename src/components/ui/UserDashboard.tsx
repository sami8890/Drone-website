...
import { User, Settings, BarChart, Sun, Moon, Upload, Bell, HelpCircle, Home, PlusCircle, Trash2 } from 'lucide-react';
...
export default function Dashboard({
    ...
    const toggleTheme = useCallback(() => {
        ...
        // Use the function form of setActivityLogs
        setActivityLogs((prevLogs) => {
            const newLog: ActivityLog = {
                id: Date.now().toString(),
                action: `Changed theme to ${newTheme} mode`,
                timestamp: new Date(),
            };
            const updatedLogs = [newLog, ...prevLogs.slice(0, 9)];
            setLocalStorage("activityLogs", updatedLogs);
            return updatedLogs;
        });
    }, [theme]);

    // The addActivityLog function can remain as is
    ...
    const toggleTask = useCallback((id: string) => {
        setTasks((prevTasks) => {
            ...
            addActivityLog(
                `${task?.completed ? "Completed" : "Uncompleted"} task: ${task?.title}`
            );
            return updatedTasks;
        });
    }, [addActivityLog]);
